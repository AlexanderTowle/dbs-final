insert into customer values ('00437650', 'dsandoval@conncoll.edu', '832-801-8753', 'Danna', 'Sandoval');
insert into customer values ('00428232', 'atowle@conncoll.edu', '860-765-8080', 'Alexander', 'Towle');
insert into customer values ('00437651', 'tbecker@conncoll.edu', '860-123-4567', 'Tim', 'Becker');
insert into customer values ('00437652', 'cchung@conncoll.edu', '860-231-4500', 'Christine', 'Chung');
insert into customer values ('00437653', 'rpeitzsc@conncoll.edu', '860-231-1234', 'Robert', 'Peitzsch');
insert into customer values ('00437623', 'parker@conncoll.edu', '860-801-9999', 'Gary', 'Parker');
insert into trans values ('00000001', '153.62', '2233-0101-8123-4666', '2024-04-25','12', 'Bank St', 'New London', 'CT', '06320');
insert into trans values ('00000002', '43.14', '1234-5555-7893-0111', '2024-04-24','12', 'Bank St', 'New London', 'CT', '06320');
insert into trans values ('00000003', '223.02', '4243-7766-1414-0916', '2024-04-24','270', 'Mohegan Ave', 'New London', 'CT', '06320');
insert into trans values ('00000004', '7.23', '4312-7761-1413-0077', '2024-03-24','87', 'Main St', 'Boston', 'MA', '02112');
insert into trans values ('00000005', '13.23', '4312-7761-1413-0077', '2024-03-28','23', 'Travis St', 'Houston', 'TX', '77080');
insert into trans values ('00000006', '231.87', '8765-0909-4444-0333', '2024-04-01','123', 'High St', 'Boston', 'MA', '02120');
insert into trans values ('00000007', '32.97', '1234-5555-7893-0111', '2023-12-28','67', 'Bank St', 'Austin', 'TX', '78707');
insert into trans values ('00000008', '96.23', '0044-4246-9843-0509', '2024-01-31','57', 'Grove St', 'Portland', 'OR', '97035');
insert into trans values ('00000009', '32.65', '8609-6146-6666-4201', '2024-02-12','57', 'Grove St', 'Portland', 'OR', '97035');
insert into trans values ('000000010', '78.01', '8300-9090-4356-8700', '2023-10-23','87', 'Main St', 'Boston', 'MA', '02112');
insert into product values ('12345678', '012345678915', '2 Liter', 'Plastic', 'Soda', 'Coca-Cola');
insert into product values ('23456789', '089345543215', '1 Gallon', 'Plastic', 'Milk', 'Fairlife');
<<<<<<< Updated upstream
insert into product values ('87654321', '009123321435', '1 Liter', 'Plastic', 'Drinking Water', 'Aquafina');
insert into product values ('44433312', '012345678925', '500 mg', 'Plastic', 'Acetaminophen', 'Tylenol');
insert into product values ('00123476', '009876909095', '11 oz', 'Plastic', 'Dishwasher Pods', 'Cascade');
insert into product values ('12007645', '055050943218', '5 lb', 'Plastic', 'Poultry', 'Tyson');
insert into product values ('89120112', '055443338985', '8 oz', 'Plastic', 'Dishwashing Soap', 'Dawn Ultra');
=======
insert into product values ('87654321', '009123321435', '18 Fl Oz', 'Plastc', 'Dish Soap', 'Dawn Ultra');
insert into product values ('44433312', '012345678925', '1 Liter', 'Platic', 'Drinking Water', 'Aquafina');
insert into product values ('00123476', '009876909095', '5 Lb', 'Plastic', 'Poulty', 'Tyson');
insert into product values ('12007645', '055050943218', '11 Oz', 'Plastic', 'Dishwasher Pods', 'Cascade');
insert into product values ('89120112', '055443338985', '500 mg', 'Cardboard', 'Acetaminophen', 'Tylenol');
>>>>>>> Stashed changes
insert into store values ('12', 'Bank St', 'New London', 'CT', '06320', '06:00:00', '23:00:00');
insert into store values ('270', 'Mohegan Ave', 'New London', 'CT', '06320', '06:00:00', '23:00:00');
insert into store values ('87', 'Main St', 'Boston', 'MA', '02112', '07:00:00', '22:00:00');
insert into store values ('23', 'Travis St', 'Houston', 'TX', '77080', '06:00:00', '22:00:00');
insert into store values ('123', 'High St', 'Boston', 'MA', '02112', '08:00:00', '24:00:00');
insert into store values ('67', 'Bank St', 'Austin', 'TX', '78707', '08:00:00', '01:00:00');
insert into store values ('57', 'Grove St', 'Portland', 'OR', '97035', '00:00:00', '24:00:00');
insert into brand values ('Aquafina', 'PepsiCo');
insert into brand values ('Fairlife', 'The Coca-Cola Company');
<<<<<<< Updated upstream
insert into brand values ('Coca-Cola', 'The Coca-Cola Company');
insert into brand values ('Dawn Ultra', 'Procter & Gamble');
insert into brand values ('Cascade', 'Procter & Gamble');
insert into brand values ('Tylenol', 'Johnson & Johnson');
insert into brand values ('Tyson', 'Tyson Foods');
insert into vendor values ('PepsiCo', '860-961-4666');
insert into vendor values ('The Coca-Cola Company', '860-123-0333');
insert into vendor values ('Procter & Gamble', '860-324-9800');
insert into vendor values ('Johnson & Johnson', '860-866-4893');
insert into vendor values ('Tyson Foods', '860-666-6789');
=======
insert into brand values ('Pepsi', 'PepsiCo');
insert into brand values ('Coca-Cola', 'The Coca-Cola Company');
insert into brand values ('Dawn Ultra', 'Procter & Gamble');
insert into brand values ('Tyson', 'Tyson Foods');
insert into brand values ('Cascade', 'Procter & Gamble');
insert into brand values ('Tylenol', 'Johnson & Johnson');
insert into vendor values ('PepsiCo', '860-961-4666');
insert into vendor values ('The Coca-Cola Company', '860-123-0333');
insert into vendor values ('Procter & Gamble', '860-874-9000');
insert into vendor values ('Tyson Foods', '860-321-9800');
insert into vendor values ('Johnson & Johnson', '860-765-3421');
>>>>>>> Stashed changes
insert into member values ('00437650', '000020');
insert into member values ('00428232', '000320');
insert into member values ('00437651', '001203');
insert into inventory values ('', '', '', '', '', '', '', '', '', '', '');
insert into purchased values ('00000001', '00437650');
insert into purchased values ('00000009', '00437651');
insert into ordered values ('12345678', '00000001');
insert into ordered values ('23456789', '00000009');
insert into sells_to values ('PepsiCo', '87', 'Main St', 'Boston', 'MA', '02112');
insert into appliance values ('', '');
insert into gas values ('', '');
insert into electric values('', '');
insert into food values('', '');
insert into beverage values('', '', '');
insert into soda values('12345678', '0');
insert into water values ('', '', '');
insert into comestible values ('', '');
insert into meat values ('', '');
insert into produce values ('', '');
insert into other values ('');
insert into cleaner values ('', '');
insert into dry_clean values ('', '');
insert into wet_clean values ('', '');
insert into drug values ('', '');